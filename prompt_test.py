from transformers import BertTokenizer, T5Tokenizer, RobertaTokenizer
import os
import argparse
import logging
import torch
...
def inference():
    ...
    parser.add_argument(
        "--bert_checkpoint_dir",
        default='./checkpoint/save_prompt_bert_model_best.pt',
        type=str,
        help="the path of fine-tuned bert checkpoint.",
    )
    parser.add_argument(
        "--t5_checkpoint_dir",
        default='./checkpoint/save_prompt_t5_model_best.pt',
        type=str,
    ...
    log_name = args.model_name + '_test_prompt'
    ...
    with open('output/20195199_1.csv', 'w', encoding='utf-8') as fw:
        for item in res:
            fw.write(str(item) + '\n')
    print('saved!')
